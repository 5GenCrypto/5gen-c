...
if [[ $# != 4 && $# != 5 ]]; then
    echo "Usage: ggm-prf-lz.sh <nprgs> <symlen> <keylen> <secparam> [circuit-dir]"
    exit 1
fi
...
args="--verbose --npowers ${npowers} --sigma --symlen ${symlen} --smart --mmap CLT --scheme LZ --secparam ${secparam} ${circuit}"
